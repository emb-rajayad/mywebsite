...
const sortOptions = [
  ...
  { name: 'Price: Low to High', href: '/products/?sort=desc', current: false },
  { name: 'Price: High to Low', href: '/products/?sort=asc', current: false },
]
const subCategories = [
...
var parameter = '';
if(route.query.sort){
  var parameter = '?sort=' + route.query.sort;
} else if(route.query.limit) {
  var parameter = '?limit=' + route.query.limit;
}
console.log(route.query);
const {status, data: products} = await useFetch("https://fakestoreapi.com/products/"+ parameter, {
    lazy: false
})
const url = '/products/';
        ...
        <section aria-labelledby="products-heading" class="pb-24 pt-6">
          ...
          <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
            ...
            <div class="lg:col-span-3">
                <!-- <div class="inset-y-0 left-0 w-3/12">Fashion Color</div> -->
...
<div v-else class="bg-white">
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8"> 
      <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
        <div v-for="product in products" class="group">
            <a  :key="product.id" :href="`/products/${product.id}`">
          <div class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
            <img :src="product.image" :alt="product.title" class=" group-hover:opacity-75" />
          </div>
          <h3 class="mt-4 text-sm text-gray-700">{{ product.title }}</h3>
          <p class="mt-1 text-lg font-medium text-gray-900">${{ product.price }}</p>
        </a>
        <div class="bootom-container">
          <button type="submit" class=" flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-1 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"><IconsPixelarticonsCart /> &nbsp;Add to cart</button>
          <span class="wishlist"><IconsMaterialSymbolsLightFavoriteOutline /></span>
        </div>
